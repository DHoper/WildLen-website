...
const fetchUserData = async () => {
  if (props.email === "self") {
    ...
    const responseData = await getUser(props.email);
...
<template>
  <div v-if="userData && formData" class="flex-1 bg-stone-100 flex items-center justify-center py-10">
    <div class="flex flex-col gap-6 items-center bg-white p-8 shadow-xl w-[30rem] border-2 border-stone-700">
      ...
      <div class="w-full flex flex-col items-center gap-4">
        <div class="relative flex flex-col items-center gap-3">
          ...
          <div v-else class="relative">
            <input v-model="formData.username" @blur="validateInput('username')" placeholder="2 ~ 7字"
              class="text-lg border-2 p-2" :class="formInputInvalid.username
                ? 'border-stone-600'
                : 'border-red-600'
                " />
        ...
        <div class="flex flex-col items-center gap-3">
          ...
          <div v-else class="relative">
            <input v-model="formData.email" @blur="validateEmail" placeholder="有效信箱" class="text-lg border-2 p-2"
              :class="formInputInvalid.email.valid &&
        ...
        <div v-if="isEditing" class="flex flex-col items-center gap-4">
          ...
          <div class="flex flex-col gap-5">
            <div class="relative">
              <input type="password" v-model="formData.password" @blur="validateInput('password')" placeholder="6 ~ 15字"
                class="text-lg border-2 p-2" :class="formInputInvalid.password
                  ? 'border-stone-600'
                  : 'border-red-600'
                  " />
            ...
            <div class="relative">
              <input type="password" v-model="formData.passwordConfirm" @blur="validateInput('passwordConfirm')"
                placeholder="6 ~ 15字" class="text-lg border-2 p-2" :class="formInputInvalid.passwordConfirm
                  ? 'border-stone-600'
                  : 'border-red-600'
                  " />
        ...
        <div class="w-full flex flex-col items-center gap-3 flex-1">
          ...
          <div
            ...
            <div v-else class="flex gap-2 items-center justify-center flex-wrap" v-for="topic in topicTags">
              <label v-for="tag in topic.tags" class="border-2 border-stone-700 px-2 py-1" :style="formData.selectedTags.includes(tag)
        ...
        <div class="flex flex-col items-center gap-4 w-full">
          ...
          <div class="w-full h-40 overflow-auto p-2 border-2 border-stone-600 whitespace-pre-wrap" v-if="!isEditing"
            :class="formData.intro ? '' : 'text-stone-500 italic'">
            {{ formData.intro || "尚未填寫自介內容" }}
          </div>
          <textarea v-else v-model="formData.intro" placeholder="10 ~ 100字" maxlength="100"
            class="w-full h-40 overflow-auto p-2 border-2 border-stone-600 resize-none"></textarea>
        </div>
      </div>
      <div class="w-full flex gap-8">
        <button v-if="props.email === 'self'" @click="submitEdit"
